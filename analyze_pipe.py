...
for fp in [r'C:\Intel\Desktop\game01\assets\Tiles\Style 1\PipeStyle1.png']:
    ...
    print(f'Color type: {r["color_type"]}')
    
    # Count transparent vs visible
    vis = sum(1 for row in rows for p in row if len(p)>=4 and p[3]>0)
    total = w*h
    print(f'Visible pixels: {vis}/{total} ({vis/total*100:.1f}%)')
    
    print(f'32-pixel columns: {"YES" if w%32==0 else "NO"} (rem={w%32})')
